...
Vault·Terraform 공용 플랫폼의 최신 코드를 이 저장소의 `main`에서 통합 관리합니다. 원본 브랜치와 열린 PR은 이전 저장소에 보존되어 있습니다. [저장소 분리 안내](REPOSITORY.md)를 참고하세요.

배포 전 `SUBNET_ID`, `SECURITY_GROUP_ID`, 승인된 `APPROVED_AMI_ID`를 본인 환경의 값으로 지정해야 합니다. 실제 운영 대상의 기본값은 제거했습니다. 아래의 `000...` 또는 `xxx...` 식별값은 실행 가능한 값이 아닌 예시입니다.
...
```bash
ROUTE53_HOSTED_ZONE_ID=Z07579811BJW2L1U58CO1 \

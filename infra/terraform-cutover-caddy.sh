...
if (( EUID != 0 )); then
  echo "terraform-cutover-caddy.sh는 root로 실행해야 합니다." >&2
  exit 1
fi
...
[[ -n "$LAB_HOST" ]] || LAB_HOST="$(read_env_value LAB_HOST)"
[[ -n "$LAB_LEGACY_HOST" ]] || LAB_LEGACY_HOST="$(read_env_value LAB_LEGACY_HOST)"
...
[[ -n "$LAB_HOST" ]] || { echo "LAB_HOST가 필요합니다." >&2; exit 1; }
...
PUBLIC_SMOKE=true LAB_HOST="$LAB_HOST" \
  "${CURRENT_DIR}/infra/terraform-smoke-deployment.sh"

trap - ERR
rm -f "$backup_file"
echo "Terraform Lab Caddy 전환과 외부 HTTPS/WSS smoke를 완료했습니다."
